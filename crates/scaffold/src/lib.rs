...
pub fn template_description(name: &str) -> Option<&'static str> {
    match name {
        ...
        "oracle-consumer" => Some("reads a price from a configurable oracle contract and uses it in a calculation"),
        "soulbound" => Some("mintable, non-transferable (soulbound) token bound to an address"),
...
mod tests {
    ...
    fn lists_all_bundled_templates() {
        assert_eq!(
            available_templates(),
            vec!["amm", "atomic-swap", "crowdfund", "escrow", "governance", "hello-world", "multisig", "nft", "oracle-consumer", "soulbound", "token", "vesting"]
        );
    }
